...
void account_info(Bank& bank)
{
    ...
    if (requested_account != 0)
    {
        ...
        if (index >= 0)
        {
            ...
            cout << "   First name: " << acc->getCustomer().getName().getFirstname() << endl;
            cout << "   Last name: " << acc->getCustomer().getName().getLastname() << endl;
            cout << "   SSN: " << acc->getCustomer().getSSN() << endl;
            cout << " Balance is: " << fixed << setprecision(2) << acc->getBalance() << endl;
        }
        else {
            cout << "Error. Account number \"" << requested_account << "\" is not found." << endl;
...
void print_accts(Bank& bank)
{
    ...
    if (outfile)
    {
        for (int i = 0; i < bank.getAccountsNumber(); i++)
        {
            Account* acc = bank.getAccount(i);

            outfile << acc->getCustomer().getName().getFirstname();
            outfile << "\t";
            outfile << acc->getCustomer().getName().getLastname();
            outfile << "\t";
            outfile << acc->getCustomer().getSSN();
            outfile << "\t";
